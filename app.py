...
docx = None  # python-docx not available in this environment


ALLOWED_CLASSIFICATIONS = {"Very Low", "Low", "Moderate", "High", "Very High"}
# Single source of truth: change this to switch the default model used by the app
...
def save_uploaded_file_to_temp(uploaded_file) -> str:
    """Persist the uploaded file to a temporary path and return the path."""
    file_suffix = Path(uploaded_file.name).suffix.lower() or ""
    with tempfile.NamedTemporaryFile(delete=False, suffix=file_suffix) as tmp_file:
        tmp_file.write(uploaded_file.getbuffer())
        return tmp_file.name
...
def _extract_bullet_lines(text: str) -> List[str]:
    """Grab lines that look like bullet points as proxy for responsibilities/skills."""
    bullets: List[str] = []
    bullet_prefixes = ("- ", "• ", "* ", "– ", "— ", "· ")
    for raw_line in text.splitlines():
        line = raw_line.strip()
...
def _heuristic_title_and_skills_from_text(text: str) -> Dict[str, Any]:
    ...
    if skills_section_indices:
        ...
        for ln in lines[start: start + 100]:
            if not ln:
                break
            # Stop on obvious next section heading (all caps words or colon-ended)
            if (ln.isupper() and len(ln.split()) <= 6) or ln.endswith(":"):
                break
            # Split comma-separated skill lists
            if "," in ln:
                parts = [p.strip() for p in ln.split(",") if p.strip()]
                skills.extend(parts)
            else:
    ...
    # If no skills section found, try bullet lines as proxy for responsibilities/skills
    if not skills:
        skills = _extract_bullet_lines(text)

    # If still empty, fall back to first few non-empty lines (avoid personal details)
    if not skills:
        candidate_lines: List[str] = []
        for ln in lines[:120]:
            if any(x in ln.lower() for x in ["email", "phone", "address", "linkedin", "github"]):
...
def _looks_like_heading(line: str) -> bool:
    ...
    if not stripped:
        return False
    # All caps short lines or colon-ended words
    return (stripped.isupper() and len(stripped.split()) <= 6) or stripped.endswith(":")
...
def extract_experience_and_education(raw_text: str) -> Dict[str, Any]:
    ...
    # Locate Experience-like sections
    exp_section_idx = None
    for i, ln in enumerate(lines):
        low = ln.lower()
        if any(h in low for h in ["experience", "work experience", "professional experience", "employment"]):
    ...
    while i < len(lines):
        ...
        # Start new entry on date range line
        if DATE_RANGE_REGEX.search(ln) or (YEAR_REGEX.search(ln) and len(ln) <= 72):
            if current_entry:
                experience_entries.append(current_entry)
            current_entry = {
            ...
            # Likely next section encountered
            experience_entries.append(current_entry)
            current_entry = None
            # do not break; there may be more items
        else:
            # Attach probable title/company lines if not bullets and reasonably short
            if current_entry and len(ln) <= 120:
                # Prepend as title/company if not set already
                if "title_company" not in current_entry:
                    current_entry["title_company"] = ln.strip()
                else:
                    # If line looks informative and not contact
                    if not any(x in ln.lower() for x in ["email", "phone", "linkedin", "github", "www."]):
                        current_entry.setdefault("context", []).append(ln.strip())
        i += 1
    if current_entry:
    ...
    # Parse education/training lines
    education_items: List[str] = []
    if education_section_idx is not None:
        for ln in lines[education_section_idx + 1 : education_section_idx + 60]:
            if _looks_like_heading(ln):
    ...
    # Derive job titles from experience headers/title lines
    title_candidates: List[str] = []
    for e in experience_entries[:10]:
        header = e.get("title_company") or e.get("header") or ""
        if header:
...
def build_recency_weighted_summary(extracted: Dict[str, Any]) -> str:
    ...
    # Experience: most recent first, limit to top 3 roles and up to 5 bullets each
    if sections.get("experience"):
        lines.append("\nExperience (most recent first):")
        for role in sections["experience"][:3]:
            header = role.get("title_company") or role.get("header") or ""
...
def parse_resume(file_path: str) -> Optional[Dict[str, Any]]:
    """Extract resume data using pyresparser when available, otherwise fallback heuristics."""
    if HAS_PYRESPARSER and ResumeParser is not None:
        try:
            parser = ResumeParser(file_path)
            extracted = parser.get_extracted_data()
    ...
    # Fallback path: extract raw text and infer title/skills heuristically
    suffix = Path(file_path).suffix.lower()
    text = ""
    if suffix == ".pdf":
        text = _extract_text_from_pdf(file_path)
    elif suffix == ".docx":
        # Best-effort: .docx parsing is unavailable without python-docx
        text = ""
    else:
        try:
            with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
...
def build_openai_client() -> Optional[OpenAI]:
    """Create an OpenAI client if the API key is present in environment."""
    if not os.getenv("OPENAI_API_KEY"):
        return None
    return OpenAI()
...
def classify_role_with_llm(
    ...
    """Send prompts to the LLM and return a dict with 'classification' and 'rationale'."""
    system_prompt = (
        "You are an expert career analyst. Your task is to evaluate a job role based on its title "
        "and responsibilities to classify the likelihood of its tasks being automated by AI.\n\n"
        "Analysis Framework:\n"
        "For this analysis, \"current AI\" refers to models capable of advanced text/code generation, "
        "data analysis, and pattern recognition. It does not include physical robotics or Artificial General "
        "Intelligence (AGI).\n\n"
        "Classification Rubric (5 levels):\n\n"
        "Very High: Core tasks are standardized, repetitive, and strongly template- or rule-driven; "
        "outputs are easily verifiable; minimal human judgment or stakeholder interaction. Examples: "
        "data entry, basic labeling, routine report generation without decision-making.\n\n"
        "High: Tasks are largely data-centric or pattern-based with occasional variation; requires limited "
        "human oversight or light domain context; heavy tool-based workflows. Examples: scripted customer "
        "support, basic QA, junior content generation following clear briefs.\n\n"
        "Moderate: A mix of automatable activities and tasks requiring non-trivial judgment, strategy, or "
        "meaningful interpersonal work (e.g., stakeholder alignment, prioritization). Examples: product "
        "analysis, data analysis with stakeholder reporting, account management.\n\n"
        "Low: Core responsibilities rely on creativity, ambiguous problem-solving, nuanced communication, or "
        "decision-making in contexts with incomplete information; automation can assist but not replace. "
        "Examples: senior consulting, therapy, classroom teaching, complex sales.\n\n"
        "Very Low: Predominantly non-automatable due to substantial physical interaction, on-site skilled work, "
        "or high-stakes leadership requiring political navigation and broad accountability. Examples: skilled "
        "trades (electrician, plumber), executive leadership with cross-org accountability."
    )

    enriched_section = f"\nEnriched Context:\n{enriched_context}" if enriched_context else ""
    user_prompt = (
        f"""Your Task:
Analyze the following resume-derived context, weighing more recent roles more heavily. Classify the role using ONLY one of: Very Low, Low, Moderate, High, Very High. Then, explain your reasoning in 2-4 sentences, referencing specific responsibilities or skills from the provided text.
...
Input (excerpt):
  ...
  Responsibilities/Skills: Enter invoice data into ERP; validate totals; copy/paste from PDFs; update spreadsheets using templates; weekly KPI dashboard refreshes.
Output:
  {{"classification": "Very High", "rationale": "Tasks are highly repetitive, template-driven, and easily verified. Minimal judgment or stakeholder interaction is required, making them highly automatable with current AI-assisted tooling."}}

Example 2 — Senior Product Manager:
Input (excerpt):
  Job Title: Senior Product Manager
  Responsibilities/Skills: Define product strategy; align cross-functional stakeholders; prioritize ambiguous requirements; run discovery interviews; make trade-offs under uncertainty.
Output:
  {{"classification": "Low", "rationale": "Core work emphasizes strategy, ambiguous problem-solving, and stakeholder alignment. While AI can assist with analysis and drafts, it cannot replace ownership and judgment in high-ambiguity contexts."}}

Example 3 — Customer Support Specialist:
Input (excerpt):
  Job Title: Customer Support Specialist
  Responsibilities/Skills: Use scripts and macros to resolve common tickets; escalate edge cases; de-escalate frustrated customers; document tickets.
Output:
  {{"classification": "Moderate", "rationale": "Scripted resolutions are automatable, but empathetic de-escalation and judgment on escalations still require human interaction."}}

Actual Input:

Job Title: {job_title}
Responsibilities/Skills: {skills_text}{enriched_section}

Output Format:
Your response MUST be a single, compact JSON object with exactly these keys. Do not include any other text or formatting:
  {{"classification": "Very Low|Low|Moderate|High|Very High", "rationale": "<2-4 sentence explanation>"}}
"""
    )
    ...
    # Print token usage to terminal for cost debugging
    try:
        usage = getattr(response, "usage", None)
        if usage is not None:
            prompt_tokens = getattr(usage, "prompt_tokens", None)
            ...
            # Optional cost estimate if per-1K rates are provided via env vars
            prompt_rate = os.getenv("OPENAI_PROMPT_RATE")  # USD per 1K prompt tokens
            completion_rate = os.getenv("OPENAI_COMPLETION_RATE")  # USD per 1K completion tokens
            if (
                prompt_rate
                and completion_rate
                and isinstance(prompt_tokens, int)
    ...
    # Fallback: treat entire content as classification or explanation
    classification = normalize_classification(content)
    if classification in ALLOWED_CLASSIFICATIONS:
        return {"classification": classification, "rationale": "Model returned only a label."}

    # Otherwise, try first token as label and rest as rationale
    tokens = content.split()
    if tokens:
        guessed = normalize_classification(tokens[0])
        remaining = content[len(tokens[0]):].strip()
...
def normalize_classification(raw_text: str) -> str:
    """Normalize and validate the LLM's response into one of the expected buckets."""
    if not raw_text:
        return "Uncertain"
...
def format_skills(skills_value: Any) -> str:
    """Convert a skills field (list/str/None) into a readable comma-separated string."""
    if skills_value is None:
        return ""
    if isinstance(skills_value, list):
        return ", ".join([str(s) for s in skills_value if s])
...
def main() -> None:
    ...
    # Proceed if at least one signal is present; otherwise show error
    if not job_title and not skills_text.strip():
        st.error(
            "Could not extract job title or responsibilities from the resume. Please upload a clearer PDF."
        )
    ...
    # Render responsibilities/skills as a short list
    responsibilities_list: List[str] = [s.strip() for s in (extracted.get("skills") or []) if str(s).strip()] if isinstance(extracted.get("skills"), list) else []
    if responsibilities_list:
        to_show = responsibilities_list[:12]
        st.markdown("- **Responsibilities/Skills (sample)**:")
    ...
    # Show experience and education summary
    sections = extract_experience_and_education(raw_text)
    if sections.get("experience"):
        st.markdown("- **Experience (most recent first)**:")
        for role in sections["experience"][:3]:
    ...
    with st.spinner("Evaluating role with AI…"):
        try:
            ...
            if os.getenv("DEBUG_LLM") == "1":
                ...
                print(f"skills_text: {skills_text}")
                print("enriched_context:\n" + enriched)
            # Use single in-code constant for model selection
            ...
            # Show basic diagnostics in the UI to aid debugging
            st.error("There was an error contacting the AI service. Please try again later.")
            st.caption("Debug info (model and error message):")
            st.code(f"model={MODEL_NAME}\nerror={str(e)}")
            return
